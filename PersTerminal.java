...
/**
 * This is the personalization terminal application for the Loyalty Card Project by:
...
public class PersTerminal {
	...
	public PersTerminal() {
		...
		try {
			RSAPrivateKey supermarketPrivateKey = (RSAPrivateKey) pers.getSupermarketKeyPair().getPrivate();
			session = new AppSession(supermarketPrivateKey);
		} catch (Exception e) {
			System.err.println(e.getMessage());
		}
	...
	//Get customer from database with id 
	public Customer getCustomer() {
		try {
			return pers.getCustomerByCard(session.getCardIdAsInt());
		} catch (Exception e) {
			System.err.println(e.getMessage());
			return null;
		}
	...
	// Save backoffice state
	public void save() {
		BackOffice bo = (BackOffice) pers;
		try {
			bo.save();
		} catch (Exception e) {
			System.err.println(e.getMessage());
	...
	public Customer registerNewCustomer(String cusName) throws Exception {
		return pers.registerCustomer(cusName);
	}

	public static void main(String[] args) {
		Security.addProvider(new BouncyCastleProvider());
		PersTerminal pt = new PersTerminal();
		mainmenu: while (true) {
			String command = "0";
			try {
				Thread.sleep(1000);
			...
			if (Integer.parseInt(command) == 1) {
				...
				while (true) {
					...
					name = IO.prompt("Enter customer's name... ");

					System.out.println("Card issued for " + name + "...");
				...
				try {
					cust = pt.registerNewCustomer(name);
					pt.personalize(cust.getID());
					System.out.println("Customer ID: " + cust.getID());
				} catch (Exception e) {
					System.err.println(e.getMessage());
					continue mainmenu;
				}
				System.out.println("Created customer (Id, name): " + cust.getID() + ", " + cust.getName());
				continue mainmenu;
				...
				List<Customer> customers = pt.getCustomerIds();
				for (Customer c : customers) {
					System.out.println("Customer No. " + c.getID() + ": " + c.getName());
				}
				Customer chosen = null;
				while (chosen == null) {
					int cust = Integer.parseInt(IO.prompt("Enter customer's id... "));
					if (cust == -1)
						continue mainmenu;
					for (Customer c : customers) {
			...
			//Delete a customer 
				...
				if (cust == -1)
					continue mainmenu;
				try {
					Customer c = pt.getCustomerByID(cust);
					System.out.println("Customer selected: " + c.getName());
				} catch (Exception e) {
					System.err.print("Invalid customer!");
				}
				IO.checkInt(cust);
				try {
					pt.deleteCustomer(cust);
					pt.revoke(cust);
					System.out.println("Deleted Customer with ID = " + cust + " from database!");
				} catch (Exception e) {
					System.err.println(e.getMessage());
					continue mainmenu;
				}
	...
	//Personalize card for new customer
	private void personalize(int cardID) {
		byte[] data = new byte[CONSTANTS.NAME_LENGTH]; 
		
		System.arraycopy(FormatData.toByteArray(cardID), 0, data, 0, CONSTANTS.NAME_LENGTH);
		
		Response response = com.sendCommand(CONSTANTS.INS_PERSONALIZE_WRITE, data);
		
		if (response == null) {
			System.out.println("Cannot Personalize card!");
			pers.deleteCard(cardID);
		} else if (!response.success()) {
			System.out.println("Error: " + FormatData.toHexString(response.getStatus()));
			pers.deleteCard(cardID);
		} else {
			System.out.println("Personalized Card!");
		}
	}
	...
	private Customer getCustomerByID(int cust) throws Exception {
		return pers.getCustomerByID(cust);
	}

	
	//Delete customer from database
	private void deleteCustomer(int customerID) throws Exception {
		pers.deleteCustomer(customerID);
	}
...
interface BackOfficeInt {
	Customer registerCustomer(String cusName) throws Exception;

	KeyPair getSupermarketKeyPair() throws Exception;

	void issueCard(Customer client) throws Exception;

	void deleteCard(int cardID);
		
	void deleteCustomer(int customerID);

	int getCardID(Customer client) throws Exception;
	
	short getCredits(Customer client) throws Exception;
	
	Customer getCustomerByID(int custID) throws Exception;

	Customer getCustomerByName(String name) throws Exception;

	Customer getCustomerByCard(int cardID) throws Exception;

	List<Customer> getCustomers() throws Exception;
}

class BackOffice implements BackOfficeInt {
	private Hashtable<Integer, Customer> customers;
	private Hashtable<Integer, sCard> cards;
	private KeyManager keymanager;
	private KeyPair supermarketKeyPair;
	private int lastCustomerId = 0;
	private int lastCardId = 0;
	private static final String keyExtension = "pos";

	public BackOffice(String path) throws Exception {
		keymanager = new KeyManager(path);
		boolean backofficeLoaded = false;

		if (new File("custDB.db").exists()) {
			try {
				loadBackOffice();
		...
		if (!backofficeLoaded) {
			System.out.println("Loading information failed!");
			try {
				if (new File("RSAPrivKey.pos").exists()) {
					System.out.println("Loading existing Supermarket Key...");
					supermarketKeyPair = keymanager.loadKeys(keyExtension);
				} else {
					System.out.println("Generating new Supermarket Key...");
			...
			cards = new Hashtable<Integer, sCard>();
			lastCustomerId = 0;
			lastCardId = 0;
			remoteSave();
		}

		System.out.println("Information loaded on " + lastCustomerId
				+ " customers, and " + lastCardId + " cards...");
	}

	private void remoteSave() throws Exception {
		try {
			save();
		} catch (IOException e) {
			throw new Exception("Saving BackOffice State Failed!", e);
		}
	}

	public void save() throws IOException {
		FileOutputStream fos = new FileOutputStream("custDB.db");
		ObjectOutputStream out = new ObjectOutputStream(fos);
		System.out.println("Information saved on " + lastCustomerId
				+ " customers, and " + lastCardId + " cards...");
	...
	private void loadBackOffice() throws IOException, ClassNotFoundException,
		...
		try {
			...
			throw new Exception("Loading of Supermarket Key Failed!", e);
		}

		FileInputStream fis = new FileInputStream("custDB.db");
		ObjectInputStream in = new ObjectInputStream(fis);
		lastCustomerId = in.readInt();
		lastCardId = in.readInt();
		customers = (Hashtable<Integer, Customer>) in.readObject();
		cards = (Hashtable<Integer, sCard>) in.readObject();
	}

	@Override
	public KeyPair getSupermarketKeyPair() throws Exception {
		return supermarketKeyPair;
	}

	@Override
	public int getCardID(Customer client) throws Exception {
		return client.getCardID();
	}

	@Override
	public Customer getCustomerByCard(int cardID) throws Exception {
	...
	@Override
	public void issueCard(Customer client) throws Exception {
		int cardId = ++lastCardId;
		sCard newCard = new sCard(cardId, (short) 0, client);
	...
	public void deleteCard(int cardId) {
		sCard card = cards.get(cardId);
		if (card == null) {
			System.err.println("Card "+cardId+" does not exist!");
			return;
		}
	...
	//Register new customer with name
	public Customer registerCustomer(String cusName) throws Exception {
	...
	@Override
	public short getCredits(Customer client) throws Exception {
	...
	public void deleteCustomer(int customerID) {
		...
		if (customer == null) {
			System.err.println("Customer " +customerID+ " does not exist!");
			return;
		}
...
class sCard implements Serializable {

	private int id;
	private short credits;
	private Customer customer;

	sCard(int cardId, short credits, Customer customer) {
		this.id = cardId;
	...
	public sCard clone() {
		return new sCard(id, credits, customer);
	}
...
class Customer implements Serializable {
	...
	private String name;
	private int id;
	private sCard currentCard = null;
...
class KeyManager {
	private String path;
	public static final String PUBKEY_BASENAME = "RSAPubKey.";
	public static final String PRIVKEY_BASENAME = "RSAPrivKey.";

	public KeyManager() {
		this.path = "/root/Desktop/lc/persterminal/src/persterminal";
	}
